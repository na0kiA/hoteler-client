  ...
  return (
    <div className="flex flex-col bg-base-100 shadow-xl ml-auto">
      {props.map((notification: NotificationType) => (
        <div className="card card-side" key={notification.id}>
          <div className="m-auto pl-3 pt-5">
            ...
            >
              <Image
                ...
                src={
                  notification.image ? notification.image : "/noImageHotel.png"
                }
                alt="通知を送ったユーザー画像"
                width={50}
                height={50}
          ...
          >
            <div className="flex flex-col p-5 pb-1">
              <h2 className="card-title text-sm mt-1 mb-1 font-bold">
                {notification.kind === "came_reviews" && (
                  <span>
                    <span className="italic">{notification.senderName}</span>が
                ...
                {notification.kind === "hotel_updates" && (
                  <span>{notification.hotelName}がホテルを更新しました。</span>
                )}
              </h2>
              <h3 className="text-sm mr-auto">{notification.message}</h3>
              <p className="text-ssm mt-1 italic">{notification.createdDate}</p>
            </div>
          </Link>
        </div>
      ))}
